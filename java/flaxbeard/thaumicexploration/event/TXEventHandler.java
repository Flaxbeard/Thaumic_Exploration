			...
			for (int z=-2;z<3;z++) {
				if (player.worldObj.isAirBlock((int)player.posX+x, (int)player.posY, (int)player.posZ+z)) {
					if (!player.worldObj.isAirBlock((int)player.posX+x, (int)player.posY-1, (int)player.posZ+z)) {
						if (player.worldObj.isSideSolid((int)player.posX+x, (int)player.posY-1, (int)player.posZ+z, ForgeDirection.UP)) {
							safeSpots.add(new ChunkCoordinates((int)player.posX+x, (int)player.posY, (int)player.posZ+z));
						}
					}
				}
			}
		...
		if (event.entityLiving instanceof EntityPlayer) {
			...
			if (player.onGround && hasBauble(ThaumicExploration.tentacleRing,BaublesApi.getBaubles(player))) {
				...
				if (safeSpots.size() > 0) {
					...
					minion.setPosition(safeSpots.get(rand).posX+0.5F,safeSpots.get(rand).posY,safeSpots.get(rand).posZ+0.5F);
