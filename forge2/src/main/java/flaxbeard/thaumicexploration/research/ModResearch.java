...
public final class ModResearch {
    ...
    public static void initResearch() {
        ...
        if (ThaumicExploration.allowUrn) {
		        research = new TXResearchItem("URN", "ARTIFICE", new AspectList().add(Aspect.MAGIC, 4).add(Aspect.VOID, 2).add(Aspect.WATER, 5), -7,9 , 1, new ItemStack(ThaumicExploration.everfullUrn)).setParents("INFUSION").setParentsHidden("ARCANEEAR").setConcealed().registerResearchItem();
        ...
        if (ThaumicExploration.allowFood) {
	        research = new TXResearchItem("TALISMANFOOD", "ARTIFICE", new AspectList().add(Aspect.HUNGER, 5).add(Aspect.FLESH, 4).add(Aspect.CROP, 4),-5,1, 2, new ItemStack(ThaumicExploration.talismanFood)).setParents("INFUSION","TALLOW").setConcealed().registerResearchItem();
